(function () {
    ...
    function TestDetailsModalController(
        ...
        var deleteWorkItemFromList = function(workItem) {
            switch (workItem.type) {
                case 'BUG':
                    ...
                    break;
                case 'TASK':
                    var taskToDelete = $scope.tasks.filter(
                        function(listWorkItem) {
                            return listWorkItem.jiraId === workItem.jiraId;
                        })[0];
        ...
        var searchTask = function(task) {
            ...
            TestService.getJiraTicket(task.jiraId).then(function(rs) {
                if (rs.success) {
                    ...
                    $scope.isNewTask = !($scope.newTask.jiraId ===
                        $scope.attachedTask.jiraId);
                    $scope.taskTabDisabled = false;
                } else {
                    alertify.error(rs.message);
                }
        ...
        var getTasks = function() {
            TestService.getTestCaseWorkItemsByType(test.id, 'TASK').
                then(function(rs) {
                    if (rs.success) {
                        $scope.tasks = rs.data;
        ...
        $scope.addTestComment = function(message) {
            ...
            var eventMessage = '';
            TestService.createTestWorkItem(test.id, testComment).
                then(function(rs) {
                    if (rs.success) {
                        $scope.testComments.push(rs.data);
                        eventMessage = generateActionResultMessage(
                            testComment.type, '', 'create', true);
                        addTestEvent(eventMessage);
                        alertify.success(eventMessage);
                    } else {
                        eventMessage = generateActionResultMessage(
                            testComment.type, '', 'create', false);
                        alertify.error(
                            'Failed to create comment for test "' + test.id);
                    }
                    $scope.testCommentText = '';
                })
        };
