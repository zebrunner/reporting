...
public class ZafiraListener implements ISuiteListener, ITestListener, IHookable, IInvokedMethodListener {
    ...
    public void onStart(ISuite suiteContext) {
        ...
        try {
            ...
            this.suite = zc.registerTestSuite(suiteContext.getName(), FilenameUtils.getName(suiteContext.getXmlSuite().getFileName()),
                    suiteOwner.getId());
    ...
    public void onTestStart(ITestResult result) {
        ...
        try {
            ...
            String primaryOwnerName = !StringUtils.isEmpty(configurator.getPrimaryOwner(result)) ? configurator.getPrimaryOwner(result)
                    : configurator.getOwner(result.getTestContext().getSuite());
            ...
            if (startedTest == null) {
                ...
                startedTest = zc.registerTestStart(testName, group, Status.IN_PROGRESS, testArgs, run.getId(), testCase.getId(),
                        configurator.getRunCount(result), convertToXML(configurator.getConfiguration()), dependsOnMethods, getThreadCiTestId(),
    ...
    public void onTestSkipped(ITestResult result) {
        ...
        if (result.getThrowable() != null && result.getThrowable().getMessage() != null
                && result.getThrowable().getMessage().startsWith("ALREADY_PASSED")) {
            return;
        }
