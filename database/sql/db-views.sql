...
CREATE MATERIALIZED VIEW TOTAL_VIEW AS (
 SELECT row_number() OVER () AS ID,
         ...
         case when (TEST_CONFIGS.PLATFORM IS NULL AND TEST_CONFIGS.BROWSER <> '') then 'WEB'
            when (TEST_CONFIGS.PLATFORM = '*' AND TEST_CONFIGS.BROWSER <> '') then 'WEB'
            when (TEST_CONFIGS.PLATFORM IS NULL AND TEST_CONFIGS.BROWSER = '') then 'API'
            when (TEST_CONFIGS.PLATFORM = '*'  AND TEST_CONFIGS.BROWSER = '') then 'API'
            when (TEST_CONFIGS.PLATFORM IS NULL AND TEST_CONFIGS.BROWSER IS NULL) then 'API'
  ...
  WHERE TESTS.FINISH_TIME IS NOT NULL
        ...
        GROUP BY PROJECT, OWNER_ID, OWNER_USERNAME, ENV, TEST_CONFIGS.PLATFORM, TEST_CONFIGS.BROWSER, date_trunc('month', TESTS.CREATED_AT)
  ORDER BY CREATED_AT
);
