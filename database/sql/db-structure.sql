...
CREATE TABLE IF NOT EXISTS UA_INSPECTIONS (
  ...
  FIRMWARE_REV VARCHAR(255) NOT NULL,
  HARDWARE_REV VARCHAR(255) NOT NULL,
  BATTERY_LEVEL INT NOT NULL,
  MODIFIED_AT TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CREATED_AT TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (ID));
CREATE TRIGGER update_timestamp_ua_inspections BEFORE INSERT OR UPDATE ON UA_INSPECTIONS FOR EACH ROW EXECUTE PROCEDURE update_timestamp();
...
CREATE MATERIALIZED VIEW BIMONTHLY_VIEW AS (
  ...
  FROM TESTS INNER JOIN
    ...
    TEST_CASES ON TESTS.TEST_CASE_ID=TEST_CASES.ID INNER JOIN
    TEST_CONFIGS ON TESTS.TEST_CONFIG_ID = TEST_CONFIGS.ID INNER JOIN
  ...
  GROUP BY TEST_RUNS.ID, USERS.ID, TEST_CONFIGS.PLATFORM, TEST_CONFIGS.BROWSER, JOBS.JOB_URL, JOBS.NAME, TEST_SUITES.USER_ID);
