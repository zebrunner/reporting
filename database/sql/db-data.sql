...
STARTED::date AS "CREATED_AT"
  ...
  ''<a href="''||UPSTREAM_JOB_URL||''" target="_blank">''||UPSTREAM_JOB_NAME||''</a>'' AS "NAME",
        ...
        SUM(FAILED) as "FAIL",
        SUM(SKIPPED) as "SKIP",
  ''<a href="#{jenkinsURL}/job/Management_Jobs/job/smartJobsRerun/buildWithParameters?token=ciStart&upstream_job_id=''||UPSTREAM_JOB_ID||''&upstream_job_build_number=''||UPSTREAM_JOB_BUILD_NUMBER||''&ci_user_id=''||OWNER||''&doRebuild=true&rerunFailures=false" id="cron_rerun" class="cron_rerun" target="_blank">Restart all</a>'' AS "RESTART ALL",
  ''<a href="#{jenkinsURL}/job/Management_Jobs/job/smartJobsRerun/buildWithParameters?token=ciStart&upstream_job_id=''||UPSTREAM_JOB_ID||''&upstream_job_build_number=''||UPSTREAM_JOB_BUILD_NUMBER||''&ci_user_id=''||OWNER||''&doRebuild=true&rerunFailures=true" class="cron_rerun" target="_blank">Restart failures</a>'' AS "RESTART FAILURES"
    FROM NIGHTLY_VIEW
  WHERE OWNER_ID=''#{currentUserId}''
