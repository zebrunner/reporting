...
DECLARE jira_tickets_id zafira.WIDGETS.id%TYPE;
DECLARE jira_tickets_sql zafira.WIDGETS.sql%TYPE;
DECLARE jira_tickets_model zafira.WIDGETS.model%TYPE;
...
BEGIN
	INSERT INTO zafira.DASHBOARDS (TITLE, HIDDEN) VALUES ('Nightly Regression Test', FALSE) RETURNING id INTO nightly_dashboard_id;

    nightly_total_sql :=
    'set schema ''zafira'';
     SELECT
        unnest(array[''PASSED'', ''FAILED'', ''SKIPPED'', ''KNOWN_ISSUE'', ''ABORTED'']) AS "label",
    ...
    jira_tickets_model := '{"columns" : ["PROJECT", "COUNT"]}';
    ...
    INSERT INTO zafira.WIDGETS (TITLE, TYPE, SQL, MODEL) VALUES (
    'JIRA TICKETS', 'table', jira_tickets_sql, jira_tickets_model)
    RETURNING id INTO jira_tickets_id;
	...
	INSERT INTO zafira.DASHBOARDS_WIDGETS (DASHBOARD_ID, WIDGET_ID, LOCATION) VALUES (nightly_dashboard_id, nightly_total_id, '{"x": 0, "y": 0, "height": 11, "width": 4}');
    INSERT INTO zafira.DASHBOARDS_WIDGETS (DASHBOARD_ID, WIDGET_ID, LOCATION) VALUES (nightly_dashboard_id, test_results_30_id, '{"x": 4, "y": 0, "height": 11, "width": 8}');
    INSERT INTO zafira.DASHBOARDS_WIDGETS (DASHBOARD_ID, WIDGET_ID, LOCATION) VALUES (nightly_dashboard_id, jira_tickets_id, '{"x": 0, "y": 22, "height": 11, "width": 12}');
