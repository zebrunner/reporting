...
CREATE TABLE IF NOT EXISTS `DASHBOARDS_WIDGETS` (
  ...
  `SIZE` INT UNSIGNED NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS `USER_PREFERENCES` (
  ...
  `CREATED_AT` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `user_preferences_users_id_fk_idx` (`USER_ID` ASC, `NAME` ASC),
  CONSTRAINT `fk_USER_PREFERENCES_1`
    FOREIGN KEY (`USER_ID`)
    REFERENCES `USERS` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE CASCADE)
...
CREATE TABLE IF NOT EXISTS `PERMISSIONS` (
  ...
  `MODIFIED_AT` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `CRIATED_AT` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `NAME_UNIQUE` (`NAME` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `GROUP_PERMISSIONS` (
  ...
  `CREATED_AT` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`ID`),
  INDEX `fk_GROUP_PERMISSIONS_GROUPS1_idx` (`GROUP_ID` ASC),
  INDEX `fk_GROUP_PERMISSIONS_PERMISSIONS1_idx` (`PERMISSION_ID` ASC),
