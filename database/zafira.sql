...
CREATE TABLE IF NOT EXISTS `MONITORS` (
  ...
  `REQUEST_BODY` TEXT(100000) NULL,
  `ENABLE_NOTIFICATION` TINYINT(1) NOT NULL,
  `CRON_EXPRESSION` VARCHAR(255) NOT NULL,
  `EXPECTED_RESPONSE_STATUS` INT NOT NULL DEFAULT 'I',
  `CREATED_AT` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `MODIFIED_AT` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `USER_MONITORS` (
  ...
  INDEX `MONITOR_ID_idx` (`MONITOR_ID` ASC),
  CONSTRAINT `USER_ID`
    FOREIGN KEY (`USER_ID`)
    REFERENCES `USERS` (`ID`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `MONITOR_ID`
